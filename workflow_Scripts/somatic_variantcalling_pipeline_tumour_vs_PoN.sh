...
PON="$WORKDIR/human_db/1000g_pon.hg38.vcf.gz"
GNOMAD="$WORKDIR/human_db/af-only-gnomad.hg38.vcf.gz"
...
# ----------------------------
# STEP 6: Somatic Variant Calling (Tumor + PoN)
# ----------------------------
RAW_VCF="$OUTDIR/vcf_files/${TUMOR}_pon_raw.vcf.gz"
FILTERED_VCF="$OUTDIR/vcf_files/${TUMOR}_pon_filtered.vcf.gz"
ANNOTATED_VCF="$OUTDIR/vcf_files/${TUMOR}_pon_annotated.vcf.gz"

echo "Calling somatic variants with Mutect2 (using PoN and gnomAD)..."
gatk --java-options "-Xmx16g" Mutect2 \
  -R $REF \
  -I $OUTDIR/bam_files/${TUMOR}_recal.bam \
  -tumor $TUMOR \
